...
describe('Find one movie - Integration', () => {
  ...
  it('should be able to return one movie', async () => {
    ...
    const response = await request(app).get(`/movies/${savedMovie1.id}`).send({});

    expect(response.status).toBe(200);
    expect(response.body.id).toBe(savedMovie1.id.toString());
    expect(response.body.movieId).toBe(550);
  });

  it('should not be able to return movie when its not exist', async () => {
    const response = await request(app).get(`/movies/507f1f77bcf86cd799439011`).send({});
